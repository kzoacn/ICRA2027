...
def main():
    ...
    batch = REPO / "runtime/evaluations" / args.label
    batch.mkdir(parents=True, exist_ok=False)
    snapshot = batch / "source"
    snapshot.mkdir()
    source = REPO
    shutil.copytree(source / "src/anchor", snapshot / "src/anchor",
                    ignore=shutil.ignore_patterns("__pycache__", "*.pyc"))
    for name in ("scripts/run.sh", "configs/resources.lock.json"):
    ...
    manifest = {"run_name": args.label, "created_at": now(),
                ...
                "source_snapshot": str(snapshot), "source_package": "src/anchor", "tasks": tasks,
    ...
    try:
        while pending or active:
            ...
            if pending or active:
                time.sleep(5)
        assert source_files(snapshot / "src/anchor") == frozen, "Frozen source changed."
