#!/usr/bin/env python3
"""Portable deployment commands for the frozen Route B v170 controller."""
...
def resource_paths() -> dict[str, Path]:
    return {
        ...
        "model": Path(os.environ.get("ROUTE_B_MODEL_PATH", ROOT / "resources/grounding-dino-tiny")).expanduser().resolve(),
    }
...
def check_resources(*, full: bool = False) -> dict[str, Path]:
    ...
    for name, path in paths.items():
        ...
        if errors:
            raise RuntimeError(f"{name}: {len(errors)} resource errors ({'; '.join(errors[:3])}). Run ./run.sh prepare")
    return paths
...
def configure_libero() -> None:
    ...
    if spec is None or not spec.submodule_search_locations:
        raise RuntimeError("hf-libero is missing; run setup.sh first")
    ...
    os.environ["ROUTE_B_MODEL_PATH"] = str(paths["model"])
...
def doctor(args: argparse.Namespace) -> int:
    ...
    from libero_system.integration.config import source_tree_sha256
    from libero_system.route_b import TaskCompiler
    ...
    if args.render:
        from libero_system.common.env_adapter import LiberoEnvAdapter, LiberoEnvConfig
...
def evaluate(args: argparse.Namespace) -> int:
    ...
    offline_runtime()
    from libero_system.integration.cli import main as run_cli
    from libero_system.integration.config import source_tree_sha256

    name = args.run_name or (datetime.now(timezone.utc).strftime("cloud_%Y%m%dT%H%M%SZ_") + uuid.uuid4().hex[:6])
...
if __name__ == "__main__":
    try:
        ...
        print(f"Route B deployment error: {exc}", file=sys.stderr)
        raise SystemExit(1)
