...
Route-B grasp/place executor does not yet implement a represented skill.
"""
...
class TaskSpec:
    ...
    def grasp_place_executable(self) -> bool:
        """Whether the current Route-B executor can consume this exact IR.

        This covers only skills implemented by ``RouteBController``.  Contact
        ...
        companion/group bindings; ``route_b_executable`` separately validates
    ...
    @property
    def route_b_executable(self) -> bool:
        """Whether the deployed Route-B dispatcher can execute every step.
        ...
        return route_b_execution_issue(self) is None
...
def route_b_execution_issue(spec: TaskSpec) -> str | None:
    """Return the first reason the deployed Route-B stack cannot execute ``spec``.
