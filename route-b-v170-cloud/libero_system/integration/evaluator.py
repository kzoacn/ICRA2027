...
from libero_system.integration.campaign_plan import orchestration_source_sha256
from libero_system.integration.config import EvaluationConfig, source_tree_sha256
from libero_system.integration.formal_provenance import (
    LIFECYCLE_SCHEMA,
...
from libero_system.integration.grasp_audit import (
    AuditRoute,
...
from libero_system.integration.results import (
    EpisodeKey,
    EpisodeRecord,
    JsonlResultStore,
    episode_schedule,
)
from libero_system.integration.policy_boundary_audit import (
    PolicyBoundaryAuditor,
    validate_policy_boundary_audit,
)
from libero_system.integration.video import DualViewVideoRecorder
from libero_system.integration.video_audit import audit_dual_view_video
from libero_system.perception import coerce_rgbd_frame
from libero_system.route_b import RouteBController
...
def _video_path(config: EvaluationConfig, key: EpisodeKey) -> Path:
    name = f"route-{key.route}_{key.suite}_task-{key.task_id:02d}_ep-{key.episode_index:04d}.mp4"
...
class _SensorOnlyActionDispatcher:
    """Execute Route C actions while returning observations and nothing else."""
...
def _commit_grasp_audit(
    *,
    ...
    engagement events are converted.  Route B's immutable pending snapshot is
    ...
    if pending_engagement is not None:
        ...
        if route is not AuditRoute.B:
            raise RuntimeError("Route C cannot persist a pending grasp engagement")
        if not fixed_horizon_exhausted:
            raise RuntimeError(
                "pending Route B grasp may only be closed by fixed-horizon exhaustion"
            )
...
def _route_c_trace(
    ...
    contact_provider: LiberoRouteCContactTargetProvider | None = None,
) -> dict[str, Any]:
    attempts = [
        {
...
class EvaluationDriver:
    ...
    def run(self) -> dict[str, Any]:
        ...
        formal_route_c_process: FormalRouteCPolicyProcess | None = None
        owns_bundle = False
        ...
        if formal_context is not None and self.config.route == "c":
            if self.perception_bundle is not None:
                raise FormalProvenanceError(
                    "formal Route C forbids an in-process perception bundle"
                )
            if self.config.perception_factory is not None:
                raise FormalProvenanceError(
                    "formal Route C forbids a custom in-process perception factory"
                )
        use_route_c_process = bool(
            self.config.route == "c"
            and self.perception_bundle is None
            and self.config.perception_factory is None
        )
        if use_route_c_process:
            formal_route_c_process = FormalRouteCPolicyProcess(
        ...
        try:
            ...
            if self.config.route == "b":
                ...
                route_b_policy = RouteBPolicy(RouteBController(bundle.for_route_b()))
            for ordinal, key in enumerate(pending, start=1):
                ...
                record = self._run_one(
                    ...
                    formal_route_c_process=formal_route_c_process,
                )
                if formal_context is not None:
                    formal_context.check_source()
                store.append(record)
            ...
            if formal_route_c_process is not None:
                formal_route_c_process.close()
            if owns_bundle:
    ...
    def _run_one(
        ...
        key: EpisodeKey,
        bundle: Any,
        route_b_policy: RouteBPolicy | None,
        *,
        formal_context: _FormalRunContext | None = None,
        formal_route_c_process: FormalRouteCPolicyProcess | Any | None = None,
    ) -> EpisodeRecord:
        if self.config.route == "c" and self.config.formal:
            if formal_route_c_process is None:
                raise FormalProvenanceError(
                    "formal Route C requires its spawned serialized policy process"
                )
        elif self.config.route != "c" and formal_route_c_process is not None:
            raise ValueError(
                "a formal Route C policy process cannot be used by this run"
            )
        ...
        try:
            ...
            # evaluator boundary.  Route C intentionally forwards only its
            ...
            boundary_auditor = (
                ...
                if self.config.formal or formal_route_c_process is not None
                else None
            )
            if self.config.route == "b":
                assert route_b_policy is not None
                payload = self._run_b(
                    environment,
                    route_b_policy,
                    recorder,
        ...
        while steps < environment.max_steps:
            ...
            if boundary_auditor is not None:
                ...
                if sequence_token is None:
                    raise RuntimeError(
                        "Route B did not expose the controller-owned observation token"
                    )
        ...
        *,
        ...
        if boundary_auditor is not None:
            if policy_task is None:
                raise RuntimeError("formal Route C requires the policy task envelope")
            ...
            policy_instruction = (
                boundary_auditor.adapt_route_c_task_to_instruction(policy_task)
            )
        else:
        ...
        if formal_route_c_process is not None:
            if boundary_auditor is None or policy_task is None:
                raise RuntimeError(
                    "isolated Route C execution requires a formal boundary audit"
                )
            ...
            result = formal_route_c_process.run_episode(
            ...
            attestation = validate_process_boundary_attestation(
                formal_route_c_process.boundary_attestation()
            )
            episode_receipt = attestation.get("latest_episode_receipt")
            if type(episode_receipt) is not dict:
                raise RuntimeError(
                    "isolated Route C returned no completed episode receipt"
                )
            ...
            if (
                ...
                raise RuntimeError(
                    "isolated Route C runtime receipt does not match the "
                    "environment action/observation lifecycle"
                )
            boundary_auditor.record_route_c_process_boundary(attestation)
            evaluator_success = score.finalize(environment)
            trace = dict(result.trace)
            trace["evaluator_isolation"] = score.audit()
            trace["formal_route_c_process_boundary"] = attestation
            fixed_horizon_exhausted = bool(
                not result.success
        ...
        def current_sensor_observation() -> RobotObservation:
            ...
            if boundary_auditor is not None:
                ...
                # Every real Route C callback is recorded, including repeated
