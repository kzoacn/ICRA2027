...
| Success, runtime and status tables | paper/data/episodes.jsonl; paper/data/raw_episodes.jsonl.gz; paper/scripts/analyze_results.py | 374/400 combined records from three controller versions: 380 historical episodes plus ten Long 09 episodes (8/10) and ten Goal 03 episodes (9/10); projected fields and original lines are checked against each other |
...
| Post-handle wrist motion and drawer-front bowl clearance | libero_system/route_b/drawer_transfer.py; controller.py: _motion, _set_pick_targets, _set_place_targets; experiments/goal03/ | Goal 03 improves from 3/10 to 9/10 in a complete targeted retest; public-model IK and observed drawer/bowl geometry, unchanged neural weights; geometry invariance checks and two related-task regression episodes |
| Whole-task replacement and historical preservation | paper/scripts/update_task_results.py; paper/data/full400_reference/; paper/scripts/check_paper.py | Complete initial-state coverage, unchanged scoring, per-record source hashes, twenty replaced and 380 retained episodes; not a new full-suite evaluation |
| Timing environments | experiments/long09/environment.json; experiments/goal03/environment.json; paper/data/provenance.json | Original full run on RTX 5090 and targeted retest on RTX 4090; combined runtime statistics are descriptive |
...
| Qualitative rollouts | paper/data/figure_provenance.json | Selected episodes from the original full campaign, before targeted revisions; video hashes and exact frame indices; fixed-camera crop is flipped vertically for upright display, preserving left/right; exported image hashes are recorded |
