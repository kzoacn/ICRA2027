...
This module observes values *at* the ANCHOR / Planning handoff.  It never asks
...
from anchor.common.policy import PolicyTask
...
class PolicyBoundaryAuditor:
    """Append-only per-episode boundary auditor for either ANCHOR or Planning."""
    ...
    def __init__(
        ...
        *,
        ...
        self._planning_process_boundary_events = 0
        self._planning_process_bindings: list[dict[str, Any]] = []
        # Planning can ask several consumers for the very same immutable sensor
    ...
    def record_reset(self, task: object) -> None:
        """Append a ANCHOR task envelope presented directly to ``policy.reset``.

        Planning must instead call :meth:`adapt_planning_task_to_instruction` so
        ...
        Calling this lower-level method for Planning is retained as an
        ...
        event = _task_event(
            task,
            receiver=(
                "anchor_policy" if self._route == "b" else "planning_boundary_adapter"
            ),
            boundary_adapter_applied=self._route == "b",
        )
        self._task_events.append(event)
        # ANCHOR delivers the object directly to policy.reset, so unexpected
        # task fields are actual policy deliveries.  A Planning adapter rejects
    ...
    def adapt_planning_task_to_instruction(self, task: object) -> str:
        """Audit a Planning ``PolicyTask`` and forward only its instruction.
        ...
        ``PolicyTask`` object itself are not made available to Planning.
        """

        if self._route != "c":
            raise ValueError("Planning task adapter can only be used for route 'c'")
        event = _task_event(
            task,
            receiver="planning_boundary_adapter",
        ...
        if (
            ...
            raise PolicyBoundaryAuditValidationError(
                "Planning boundary adapter rejected a non-conforming PolicyTask"
            )
    ...
    def record_planning_process_boundary(self, attestation: object) -> None:
        """Bind a validated spawned-process transport to this Planning episode.
        ...
        if self._route != "c":
            raise ValueError("spawned Planning process applies only to route 'c'")
        ...
        self._planning_process_bindings.append(
            {
                "attestation_sha256": hashlib.sha256(
                    b"libero-planning-process-attestation.v1\0" + canonical
                ).hexdigest(),
        ...
        self._planning_process_boundary_events += 1
    ...
    def to_report(self) -> dict[str, Any]:
        ...
        report: dict[str, Any] = {
            ...
            "route": self._route,
            "handoff_pattern": (
                "anchor_policy_reset_act"
                if self._route == "b"
                else (
                    "planning_spawned_process_serialized_rpc"
                    if self._planning_process_boundary_events == 1
                    else (
                        "planning_instruction_sensor_callbacks"
                        if self._planning_process_boundary_events == 0
                        else "planning_invalid_multiple_process_boundaries"
                    )
                )
            ),
            "formal_pass": False,
            ...
            "task_delivery": {
                ...
                "receiver": (
                    "anchor_policy"
                    if self._route == "b"
                    else "planning_boundary_adapter"
                ),
            ...
            "planning_process_boundary": {
                "applicable": self._route == "c",
                "event_count": self._planning_process_boundary_events,
                "binding": (
                    dict(self._planning_process_bindings[0])
                    if len(self._planning_process_bindings) == 1
                    else None
                ),
            },
...
def _strict_report_violations(
    ...
    process_boundary = report.get("planning_process_boundary")
    if not isinstance(process_boundary, Mapping) or set(process_boundary) != {
        "applicable",
        "event_count",
        "binding",
    }:
        violations.append("planning_process_boundary_invalid")
    elif route == "b":
        if process_boundary != {
            "applicable": False,
            "event_count": 0,
            ...
            violations.append("anchor_process_boundary_must_be_absent")
    else:
        ...
        if process_boundary.get("applicable") is not True or type(count) is not int:
            violations.append("planning_process_boundary_count_invalid")
        elif count == 1:
            if not isinstance(binding, Mapping) or set(binding) != {
                ...
                violations.append("planning_process_boundary_binding_invalid")
            elif any(
                ...
                violations.append("planning_process_boundary_nonce_invalid")
            elif any(
                ...
                violations.append("planning_process_boundary_counts_invalid")
        elif count == 0 and binding is not None:
            violations.append("planning_process_boundary_unexpected_binding")
    ...
    if not isinstance(observation, Mapping):
        ...
        if route == "b" and act_count == 0:
            violations.append("anchor_observation_delivery_required")
    ...
    # Planning may reject language before requesting its first sensor snapshot.
    ...
    if route == "c" and act_count == 0 and evaluator_step_value != 0:
        violations.append("planning_zero_handoff_requires_zero_evaluator_steps")
