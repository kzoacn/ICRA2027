...
def validate_grasp_attempt_counts(
    ...
    *,
    ...
    anchor_budget_records = sum(
        record.reason_code is GraspReasonCode.STEP_BUDGET_EXHAUSTED
        for record in parsed
    )
    if route is AuditRoute.B and anchor_budget_records != values["pending"]:
        raise GraspAuditValidationError(
            "ANCHOR step_budget_exhausted records must exactly equal pending count"
        )
    if values["pending"]:
        if route is not AuditRoute.B:
            raise GraspAuditValidationError(
                "only ANCHOR may persist a fixed-horizon pending engagement"
            )
        terminal = parsed[-1]
        if (
            terminal.accepted
