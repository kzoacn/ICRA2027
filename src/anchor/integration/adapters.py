"""Thin adapters from the shared sensor boundary to ANCHOR and Planning."""
...
from anchor.integration.cavity import (
    CavityGeometryError,
    align_panda_finger_axis,
    infer_cavity_frame,
)
from anchor.manipulation import (
...
from anchor.planning.controller import (
    ExecutionError,
    TerminalExecutionError,
    Verification,
)
from anchor.planning.grasp import GraspBindingError
from anchor.planning.optimizer import OptimisationError
from anchor.common.pan_affordance import (
    PanAffordanceError,
    PanHandleAffordance,
...
# weakening the Planning three-frame 6-mm evidence gate.
_FREE_RIM_SENSOR_SAFE_MAX_VERTICAL_DRIFT_M = 0.006


def to_anchor_observation(observation: RobotObservation) -> BSensorObservation:
...
class AnchorMicrowaveDoorDetector:
    ...
    def detect(
        ...
        converted = to_anchor_observation(observation)
        ...
        if kind is GoalSkillKind.CLOSE_MICROWAVE and self._fixture_surface_points_world is not None:
            from ..manipulation.microwave_interior import microwave_cavity_from_walls
        ...
        self.last_contact_detection = self._contact_trace(
            "anchor_microwave_contact_detection",
            target,
            requested_kind=kind,
    ...
    def panel_geometry(self, observation: RobotObservation):
        ...
        from ..manipulation.microwave_interior import microwave_cavity_from_walls, observed_door_panel
    ...
    def articulation_geometry(
        ...
        *,
        ...
        """Delegate sensor hinge selection while preserving ANCHOR trace."""
        ...
        if self._wall_articulation is not None and initial_is_open is True:
            ...
            # This is the same pure RGB-D/public-asset fit used by Planning,
            # called without sharing any provider state.  ANCHOR freezes its
            # own selected body cloud and selected closed slot below.
            hinge, axis, closed_slot, surface_trace = (
                LiberoPlanningContactTargetProvider._surface_refined_microwave_articulation(
        ...
        trace: dict[str, Any] = {
            "kind": "anchor_microwave_hinge_hypotheses",
            "initial_door_state": (
    ...
    def track(
        ...
        self.last_contact_track = self._contact_trace(
            "anchor_microwave_contact_track",
            refreshed,
            requested_kind=kind,
    ...
    def _select_fixture_geometry(
        ...
        trace = {
            "kind": "anchor_microwave_body_selection",
            "candidates": [
                {
...
class AnchorPolicy:
    """Sensor-only sequential dispatcher for the complete ANCHOR IR.

    Consecutive grasp/place steps retain the original ANCHOR visual servo.
    ...
    segment and must pass ANCHOR's stack, lift, and final-placement RGB-D
    gates.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.contact_policy = contact_policy or GoalContactPolicy(
            microwave_detector=AnchorMicrowaveDoorDetector(controller.perception)
        )
        ...
        self._segments: tuple[_AnchorSegment, ...] = ()
    ...
    def observation_sequence_token(self) -> int | None:
        ...
        ANCHOR can dispatch both manipulation and contact subcontrollers.
    ...
    def grasp_attempt_events(self) -> tuple[GraspAttemptEvent, ...]:
        """Episode-global mechanical engagements across ANCHOR segments."""
    ...
    def reset(self, task: PolicyTask) -> None:
        ...
        if not formal_reset_graph:
            ...
            self._execution_issue = anchor_execution_issue(self._spec)
        ...
        if self._execution_issue is not None:
            ...
            return
        self._segments = AnchorPolicy._build_segments(self._spec)
        AnchorPolicy._activate_segment(
            self,
    ...
    def act(self, observation: RobotObservation) -> PolicyDecision:
        ...
        if self._legacy_direct:
            decision = type(self.controller).act(
                self.controller,
                to_anchor_observation(observation),
            )
        ...
        if (
            ...
            if callable(observe):
                snapshot = observe(to_anchor_observation(observation), ("stove",))
        ...
        if (self._boundary_observation_sequence == 1 and self._segments
            ...
            if callable(observe) and following is not None:
                ...
                if first.kind is BSkillKind.PICK and first.selector is None:
                    ...
                    snapshot = observe(to_anchor_observation(observation), (first.subject,))
        ...
        if self._segment_index >= len(self._segments):
            decision = BControlDecision(
                ...
                "done",
                "all sensor-only ANCHOR segments completed",
            )
            self.last_decision = decision
            return self._common_decision(decision, terminal=True)
        ...
        if segment.mode == "manipulation":
            ...
            if local.status is BExecutorStatus.SUCCEEDED:
                ...
                if self._segment_index < len(self._segments):
                    AnchorPolicy._activate_segment(self)
                    ...
                    phase = "done"
                    message = "all sensor-only ANCHOR segments completed"
                decision = BControlDecision(
                    local.action,
                    status,
    ...
    @classmethod
    def _build_segments(cls, spec: BTaskSpec) -> tuple[_AnchorSegment, ...]:
        ...
        for index, step in enumerate(spec.steps):
            if step.kind in cls._CONTACT_KINDS:
                if current:
                    segments.append(
                        _AnchorSegment(current_start, tuple(current), "manipulation")
                    )
                    current = []
                segments.append(_AnchorSegment(index, (step,), "contact"))
            else:
                if not current:
                    current_start = index
                current.append(step)
        if current:
            segments.append(_AnchorSegment(current_start, tuple(current), "manipulation"))
    ...
    def _activate_segment(
        ...
        *,
        ...
        if segment.mode == "manipulation":
            ...
            if episode_state_cleared:
                ...
                if reset_counts is not None:
                    reset_counts["anchor_controller_activate"] += 1
            else:
        ...
        instruction = AnchorPolicy._contact_instruction(step)
    ...
    def _archive_controller_trace(self, segment: _AnchorSegment) -> None:
    ...
    def _archive_selector_diagnostics(self, segment: _AnchorSegment) -> None:
...
class WorkspaceCenterEntityResolver(EntityResolver):
    ...
    top drawer" into "any detected black bowl".  Planning instead validates
...
class LiberoPlanningContactTargetProvider:
    """Adapt calibrated dual-view detectors to Planning contact geometry.
...
class OSCWaypointConfig:
    ...
    # Must be tighter than PlanningController's 8 mm phase tolerance; otherwise
    ...
    # is reached, matching the independently calibrated Planning rim grasp.
...
class LiberoPlanningRobot:
    """Execute Planning optimiser waypoints through real normalized OSC actions.
    ...
    def capture_fresh_sensor_frame_at_pose(
        ...
        if self.steps_executed >= self.step_budget:
            ...
            # ``ContactAwarePlanningController.run``; otherwise the base
    ...
    def _run_cavity_rim_load_proof(self) -> ControllerFeedback:
        ...
        The outer Planning controller freezes the full LIFT goal before this
    ...
    def servo_gripper_width_at_pose(
        ...
        *,
        ...
        if (
            ...
            or self._active_grasp_candidate_id != "planning-contact-open"
        ):
            return ControllerFeedback(
                False,
                "microwave compact servo requires the typed Planning open contact",
            )
...
class ContactAwarePlanningController(PlanningController):
    ...
    def run(self, task_text: str) -> PlanningResult:
    ...
    def _require_fresh_free_rim_visibility(
        ...
        *,
        ...
        Both Planning tracking layers deliberately retain a requested entity
    ...
    def _execute_phase(
        ...
        try:
            ...
            if phase == Phase.LIFT and not retained:
                ...
                # same-phase retry in PlanningController and is caught by its
...
class AnalyticTopDownGraspProvider:
    ...
    def propose(self, scene: SceneEstimate, object_id: str) -> Sequence[GraspCandidate]:
        ...
        # same sensor formula is shared with the validated ANCHOR grasp:
...
class SensorBoundGoalSynthesizer(LabelCheckedGoalSynthesizer):
    ...
    The first LIFT request can only occur after Planning has closed the gripper
    ...
    def _effective_object_from_ee(
        ...
        # alternative.  Resetting every option to source.z made Planning retry
...
class StableSceneEstimator:
    """Maintain sensor-derived Planning identities across frame-local reordering.
    ...
    def selector_diagnostics(self) -> tuple[dict[str, object], ...]:
        """Append-only Planning selector evidence across inner track resets."""
    ...
    def last_selector_view_hint(self):
        ...
        exposes that bounded active-view request to Planning; it never inserts
    ...
    def _validated_typed_wrist_negative_visibility(
        ...
        inflation = ContactAwarePlanningController._builtin_finite_number(
        ...
        if (
            ...
            or abs(
                inflation
                - ContactAwarePlanningController._FREE_RIM_VIEW_OBB_UNCERTAINTY_M
            )
            > 1e-12
        ...
        rotation_error = ContactAwarePlanningController._rotation_distance_rad(
            predicted_world_from_camera, world_from_camera
        )
        if (
            position_error
            > ContactAwarePlanningController._FREE_RIM_VIEW_CAMERA_RIGID_POSITION_TOLERANCE_M
            or rotation_error
            > ContactAwarePlanningController._FREE_RIM_VIEW_CAMERA_RIGID_ROTATION_TOLERANCE_RAD
        ):
            return None
        return {
            "mode": value["mode"],
        ...
        *,
        ...
        if (
            ...
            or (
                ...
                and ContactAwarePlanningController._builtin_finite_number(
                    maximum_relative_error_m
                )
                is None
            )
        ...
        for camera_name, raw_points in surface_points_by_camera:
            points = np.asarray(raw_points, dtype=np.float64)
            canonical_name = (
                ContactAwarePlanningController._canonical_view_name(camera_name)
        ...
        public_tool_evidence = {
            camera_name: (
                (
                    ContactAwarePlanningController._typed_negative_panda_tool_surface_evidence
                    if typed_visibility is not None
                    else ContactAwarePlanningController._released_panda_tool_surface_evidence
                )(points, ee_pose)
            )
        ...
        if typed_visibility is not None:
            ...
            frustum = ContactAwarePlanningController._wrist_frustum_evidence(
                center,
                half,
                axes,
                wrist_model,
    ...
    def temporal_proprio_field_evidence(
        ...
        *,
        ...
        if field_axes_world is not None or surface_points_by_camera is not None:
            expected_axes = (
                ContactAwarePlanningController._strict_provider_float_array(
                    field_axes_world, (3, 3)
                )
            )
            expected_surfaces = (
                ContactAwarePlanningController._strict_provider_surface_map(
                    surface_points_by_camera,
                    canonical_policy_names=False,
                )
            )
    ...
    def _stabilize_observation(
        ...
        *,
        ...
        # The inner Planning estimator retains requested entities through
    ...
    def _without_temporal_proprio_self_component(
        ...
        The proof is installed by ``ContactAwarePlanningController`` only after
        ...
        if any(
            ...
            return scene
        surface_parser = (
            ContactAwarePlanningController._canonical_surface_map
            if typed_negative_mode
            else ContactAwarePlanningController._camera_surface_map
        )
        prior_surfaces = surface_parser(proof.get("surface_points_by_camera"))
        ...
        surface_gate = bool(
            ...
            and (
                ...
                >= ContactAwarePlanningController._FREE_RIM_VIEW_FIELD_MIN_CAMERAS
            )
            and all(
                len(points)
                >= ContactAwarePlanningController._FREE_RIM_VIEW_FIELD_MIN_POINTS_PER_CAMERA
                for points in (
                    *prior_surfaces.values(),
                    *current_surfaces.values(),
                )
        ...
        point_errors = {
            name: (
                ContactAwarePlanningController._strict_surface_alignment_error_m
                if typed_negative_mode
                else ContactAwarePlanningController._surface_alignment_error_m
        ...
        if not all(
            np.isfinite(error)
            and error
            <= ContactAwarePlanningController._FREE_RIM_VIEW_FIELD_MAX_POINT_ERROR_M
...
class SensorFormedStackHandler:
    ...
    The handler records the stable identities produced by the first Planning
    ...
    def capture_stack(
        self, goal: AtomicGoal, result: PlanningResult
    ) -> Verification:
        self._binding = None
        if (
