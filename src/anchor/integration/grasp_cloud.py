...
ships a ``piper_hand`` geometry.  Planning uses a Panda end-effector in the
...
from anchor.planning import GraspCandidate, SceneEstimate
...
class GraspCloudResultProvider:
    """Planning ``GraspProvider`` backed by an injected frozen-service call.
