...
from anchor.perception import (
    ...
    ObjectInstance,
    PerceptionConfig,
    AnchorScenePerceptionAdapter,
    PlanningSceneEstimatorAdapter,
    SensorOnlyScenePerception,
    TextureSizeGallery,
)

if TYPE_CHECKING:
    from anchor.integration.config import EvaluationConfig


class LabelSpecificPlanningSceneEstimatorAdapter(PlanningSceneEstimatorAdapter):
    ...
    of an unrelated top class such as milk.  Planning resolves same-label
    ...
    def _observe_label_specific(self, requested_labels, *, source_reference):
        ...
        if source_reference is not None:
            from anchor.planning.schema import EntityRef
...
class PerceptionBundle:
    ...
    planning_episode_cache: bool = False

    def for_anchor(self) -> AnchorScenePerceptionAdapter:
        return AnchorScenePerceptionAdapter(
            self.backend,
            box_detector=self.box_detector,
            box_extractor=self.box_extractor,
        )

    def for_planning(self, frame_provider) -> PlanningSceneEstimatorAdapter:
        return LabelSpecificPlanningSceneEstimatorAdapter(
            frame_provider,
            self.backend,
            box_detector=self.box_detector,
            box_extractor=self.box_extractor,
            enable_episode_cache=self.planning_episode_cache,
        )
...
def build_perception_bundle_from_context(
    ...
    *,
    ...
    Formal Planning uses this entry point inside its isolated policy process.
    ...
    planning_episode_cache = context.perception_backend != "gallery"
    if not needs_detector:
        return PerceptionBundle(
            backend,
            planning_episode_cache=planning_episode_cache,
        )
