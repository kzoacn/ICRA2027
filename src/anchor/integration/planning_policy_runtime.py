"""Planning policy runtime with no evaluator or simulator capabilities.

This module assembles Planning from language, sanitized sensor callbacks, a
...
from anchor.common import OSCAction, RobotObservation
...
from anchor.integration.policy_ipc import camera_content_sha256
...
_EPISODE_RUNTIME_SCHEMA = "libero-planning-episode-runtime.v1"
...
def _runtime_state_sha256(value: dict[str, int | bool]) -> str:
    ...
    return hashlib.sha256(
        b"libero-planning-runtime-initial-state.v1\0" + encoded
    ).hexdigest()


def _assemble_planning_episode_runtime(
    *,
    ...
    if initial_state != expected_initial_state:
        raise RuntimeError("new Planning episode runtime did not start empty")
    _RUNTIME_ASSEMBLY_COUNT += 1
    assembly_nonce = secrets.token_hex(32)
    identity = hashlib.sha256(
        b"libero-planning-runtime-components.v1\0"
...
def run_planning_policy_episode(
    *,
    bundle: PerceptionBundle | Any,
    instruction: str,
    current_observation: SensorProvider,
    ...
    """Execute one Planning episode from capability-limited arguments only."""

    if type(instruction) is not str or not instruction.strip():
        raise ValueError("Planning instruction must be a non-empty native string")
    if type(step_budget) is not int or step_budget < 1:
        raise ValueError("Planning step_budget must be a positive native integer")
    if not callable(current_observation) or not callable(execute_action):
        raise TypeError("Planning sensor and action capabilities must be callable")

    def frames():
        current = current_observation()
        if type(current) is not RobotObservation:
            raise TypeError("Planning sensor capability returned a non-whitelist DTO")
        return tuple(
            coerce_rgbd_frame(
                frame,
                name=name,
    ...
    assembly = _assemble_planning_episode_runtime(
        bundle=bundle,
        frames=frames,
    ...
    try:
        ...
        result = SimpleNamespace(
            success=False,
            failure=str(exc) or "fixed Planning policy horizon exhausted",
            attempts=(),
            verification=None,
            goals=(),
...
def run_planning_policy_test_episode(
    *,
    bundle: PerceptionBundle | Any,
    instruction: str,
    current_observation: SensorProvider,
    ...
    same mutable Planning runtime as a normal episode before issuing one hold.
    """

    if type(instruction) is not str or not instruction.strip():
        raise ValueError("Planning test instruction must be non-empty")
    if type(step_budget) is not int or step_budget < 1:
        raise ValueError("Planning test step_budget must be positive")
    ...
    if not feedback.accepted or assembly.robot.steps_executed != 1:
        raise RuntimeError("production-shaped Planning test action was rejected")
    return PlanningPolicyExecution(
        success=True,
        failure=None,
        steps_executed=1,
