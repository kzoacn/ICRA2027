"""Sandboxed, bounded, replay-resistant Planning policy process boundary.
...
from anchor.common.observation import RobotObservation
...
from anchor.integration.formal_process_sandbox import (
    BWRAP_PATH,
...
from anchor.integration.policy_ipc import (
    ACTION_WIRE_SCHEMA,
...
from anchor.integration.policy_result import PlanningPolicyExecution


PROCESS_INIT_SCHEMA = "libero-formal-planning-process-init.v2"
PROCESS_MESSAGE_SCHEMA = "libero-formal-planning-process-message.v2"
PROCESS_BOUNDARY_ATTESTATION_SCHEMA = (
    "libero-formal-planning-process-boundary-attestation.v2"
)
PROCESS_RUNTIME_RECEIPT_SCHEMA = "libero-formal-planning-runtime-receipt.v2"
PROCESS_EPISODE_RECEIPT_SCHEMA = "libero-formal-planning-episode-receipt.v2"
SENSOR_COMMITMENT_SCHEMA = "libero-sensor-content-commitments.v2"
EPISODE_RUNTIME_SCHEMA = "libero-planning-episode-runtime.v1"
...
_FORBIDDEN_VISIBLE_PATHS = (
    ...
    "/policy/anchor/common/env_adapter.py",
    "/policy/anchor/common/runner.py",
    "/policy/anchor/integration/campaign_manifest.py",
    "/policy/anchor/integration/campaign_plan.py",
    "/policy/anchor/integration/evaluator.py",
    "/policy/anchor/integration/results.py",
    "/policy/anchor/integration/video.py",
...
_BOOTSTRAP = (
    ...
    "from anchor.integration.planning_process import _child_bootstrap;"
    "sys.exit(_child_bootstrap(int(sys.argv[1]),sys.argv[2]=='1'))"
)


class FormalPlanningProcessError(RuntimeError):
...
def _strict_json_copy(value: Mapping[str, Any]) -> dict[str, Any]:
    try:
        ...
        raise FormalPlanningProcessError(str(exc)) from exc
...
def _init_payload(context: PerceptionFactoryContext) -> dict[str, Any]:
    if type(context) is not PerceptionFactoryContext:
        raise TypeError("formal Planning context must use the exact safe DTO")
    if (
        ...
        raise ValueError("formal Planning perception context is invalid")
    return {
...
def _context_from_init(payload: object) -> PerceptionFactoryContext:
    ...
    if type(payload) is not dict or set(payload) != keys:
        raise FormalPlanningProcessError("child init payload has invalid fields")
    if payload["schema"] != PROCESS_INIT_SCHEMA or type(payload["schema"]) is not str:
        raise FormalPlanningProcessError("child init schema is invalid")
    ...
    if (
        ...
        raise FormalPlanningProcessError("child init deployment values are invalid")
...
def _event_from_dict(value: object) -> GraspAttemptEvent:
    ...
    if type(value) is not dict or set(value) != keys:
        raise FormalPlanningProcessError("child grasp event has invalid fields")
    try:
        ...
        raise FormalPlanningProcessError(f"child grasp event is invalid: {exc}") from exc
...
def _pending_from_dict(value: object) -> PendingGraspEngagement | None:
    ...
    if type(value) is not dict or set(value) != keys:
        raise FormalPlanningProcessError("child pending engagement has invalid fields")
    try:
        ...
        raise FormalPlanningProcessError(
...
def _trace_json_copy(trace: Mapping[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        raise FormalPlanningProcessError(f"cannot encode policy trace: {exc}") from exc


def _result_core(
    result: PlanningPolicyExecution,
...
def _result_from_core(payload: object) -> PlanningPolicyExecution:
    keys = {
        "type",
        "success",
        "failure",
    ...
    if type(payload) is not dict or set(payload) != keys or payload["type"] != "result":
        raise FormalPlanningProcessError("child result has invalid fields")
    if type(payload["success"]) is not bool:
        raise FormalPlanningProcessError("child result success is not a native boolean")
    if payload["failure"] is not None and type(payload["failure"]) is not str:
        raise FormalPlanningProcessError("child result failure is invalid")
    if type(payload["steps_executed"]) is not int or not (
        0 <= payload["steps_executed"] <= MAX_POLICY_STEPS
    ):
        raise FormalPlanningProcessError("child result step count is invalid")
    if type(payload["trace"]) is not dict:
        raise FormalPlanningProcessError("child result trace is invalid")
    events = payload["grasp_attempt_events"]
    if type(events) is not list or len(events) > MAX_GRASP_EVENTS:
        raise FormalPlanningProcessError("child result grasp events are invalid")
    return PlanningPolicyExecution(
...
def _transcript_seed(
    ...
    return hashlib.sha256(
        b"libero-planning-transcript-seed.v1\0"
...
class _Transcript:
    ...
    def __init__(
        ...
        *,
        ...
        if not _hash_valid(worker_nonce) or not _hash_valid(episode_nonce):
            raise FormalPlanningProcessError("transcript nonce is invalid")
        if type(episode_index) is not int or episode_index < 1:
            raise FormalPlanningProcessError("transcript episode index is invalid")
    ...
    def _next_hash(
        ...
        *,
        ...
        return hashlib.sha256(
            b"libero-planning-transcript-link.v1\0"
    ...
    def wrap(self, core: Mapping[str, Any], *, direction: str) -> dict[str, Any]:
        if type(core) is not dict or type(core.get("type")) is not str:
            raise FormalPlanningProcessError("transcript core is invalid")
        result = {
    ...
    def unwrap(self, message: object, *, direction: str) -> dict[str, Any]:
        if type(message) is not dict:
            raise FormalPlanningProcessError("chained message must be an exact object")
        envelope = {
            "schema",
            "worker_nonce",
            "episode_nonce",
        ...
        if not envelope < set(message):
            raise FormalPlanningProcessError("chained message envelope is incomplete")
        if (
            ...
            raise FormalPlanningProcessError("stale, replayed, or reordered process message")
        core = {name: value for name, value in message.items() if name not in envelope}
        if type(core.get("type")) is not str:
            raise FormalPlanningProcessError("chained message type is invalid")
        ...
        if message["transcript_sha256"] != expected:
            raise FormalPlanningProcessError("process transcript hash is invalid")
        self.next_sequence += 1
...
def _observation_envelope(
    ...
    try:
        ...
        raise FormalPlanningProcessError(f"cannot encode observation: {exc}") from exc
...
def _decode_observation(
    ...
    if type(value) is not dict or "observation" not in value:
        raise FormalPlanningProcessError("observation envelope is invalid")
    ...
    if set(core) != {"type", "commitments"} or core["type"] != "observation":
        raise FormalPlanningProcessError("observation chain fields are invalid")
    try:
        observation = deserialize_policy_observation(wire)
    except PolicyIPCValidationError as exc:
        raise FormalPlanningProcessError(f"invalid parent observation: {exc}") from exc
    commitments = sensor_content_commitments(observation)
    if core["commitments"] != commitments:
        raise FormalPlanningProcessError("observation sensor commitment is invalid")
...
def _mount_status(destination: str) -> dict[str, Any]:
    ...
    if not matches:
        raise FormalPlanningProcessError(f"expected sandbox mount is absent: {destination}")
...
def _proc_starttime(pid: int | str) -> int:
    ...
    if value <= 0:
        raise FormalPlanningProcessError("process starttime is invalid")
    return value
...
def _open_fd_manifest(protocol_fd: int, *, accelerator: bool = False) -> list[dict[str, Any]]:
    ...
    if [item for item in records if item["kind"] != "accelerator"] != expected:
        raise FormalPlanningProcessError(f"child has unexpected open descriptors: {records}")
    return records
    ...
    *,
    ...
    if context.perception_backend in {"auto", "grounding-dino"}:
        if context.perception_model is None:
            from anchor.perception.grounding_dino import DEFAULT_GROUNDING_DINO_TINY
    ...
    for readonly_path in (
        ...
        if not by_destination[readonly_path]["read_only"]:
            raise FormalPlanningProcessError(f"sandbox mount is not read-only: {readonly_path}")
    if model_root is not None and not by_destination[str(model_root)]["read_only"]:
        raise FormalPlanningProcessError("model repository mount is not read-only")
    if any(not by_destination[path]["read_only"] for path in SYSTEM_RUNTIME_FILES):
        raise FormalPlanningProcessError("fixed system runtime file is not read-only")
    system_runtime_files = [
        {
            "destination": path,
    ...
    if accelerator:
        if not by_destination["/usr/lib/wsl"]["read_only"]:
            raise FormalPlanningProcessError("GPU driver runtime must be read-only")
    ...
    if any(not item["read_only"] for item in identity_files):
        raise FormalPlanningProcessError("fixed identity file is not read-only")
    ...
    if any(
        ...
        for name in (
            "anchor.common.env_adapter",
            "anchor.common.runner",
            "anchor.integration.evaluator",
            "anchor.integration.results",
            "anchor.integration.video",
        )
    ):
        raise FormalPlanningProcessError("child eagerly loaded evaluator-side modules")
    if any(forbidden.values()):
        raise FormalPlanningProcessError("child can access a forbidden policy path")
    if os.getpid() != 1 or os.getppid() != 0 or proc_pids != ["1"]:
        raise FormalPlanningProcessError("child is not isolated as PID 1 with private proc")
    if interfaces != ["lo"]:
        raise FormalPlanningProcessError("child network namespace has non-loopback interfaces")
    if cap_eff != "0000000000000000":
        raise FormalPlanningProcessError("child retained Linux capabilities")
    if socket.gethostname() != SANDBOX_HOSTNAME:
        raise FormalPlanningProcessError("child UTS namespace hostname is invalid")
...
def _validate_sensor_commitment(value: object) -> dict[str, Any]:
    ...
    if type(value) is not dict or set(value) != keys:
        raise FormalPlanningProcessError("episode sensor commitment has invalid fields")
    ...
    if (
        ...
        raise FormalPlanningProcessError("episode sensor commitment is invalid")
    return value
...
def _validate_episode_runtime_receipt(value: object) -> dict[str, Any]:
    ...
    if type(value) is not dict or set(value) != keys:
        raise FormalPlanningProcessError(
            "Planning episode runtime receipt has invalid fields"
        )
    ...
    if (
        ...
        != _json_sha256(
            initial_state,
            b"libero-planning-runtime-initial-state.v1\0",
        )
        or type(value["final_action_count"]) is not int
        or not 0 <= value["final_action_count"] <= MAX_POLICY_STEPS
    ):
        raise FormalPlanningProcessError(
            "Planning episode runtime receipt values are invalid"
        )
    return value
...
def _validate_episode_receipt(value: object) -> dict[str, Any]:
    ...
    if type(value) is not dict or set(value) != keys:
        raise FormalPlanningProcessError("Planning episode receipt has invalid fields")
    integer_fields = (
        "episode_index",
        "message_count",
        "observation_count",
    ...
    if (
        ...
        raise FormalPlanningProcessError("Planning episode receipt values are invalid")
    ...
    if (
        ...
        raise FormalPlanningProcessError("Planning episode transcript lists are invalid")
...
def _validate_runtime_receipt(value: object) -> dict[str, Any]:
    ...
    if type(value) is not dict or set(value) != keys:
        raise FormalPlanningProcessError("Planning runtime receipt has invalid fields")
    ...
    if (
        ...
        not in {"production_planning_policy_episode", "embedded_test_fixture"}
        ...
        raise FormalPlanningProcessError("Planning runtime receipt values are invalid")
    if type(value["forbidden_path_access"]) is not dict or set(
        ...
        raise FormalPlanningProcessError("Planning forbidden-path receipt is invalid")
    if type(value["mount_status"]) is not list or len(value["mount_status"]) < 6:
        raise FormalPlanningProcessError("Planning mount receipt is invalid")
    ...
    for item in value["mount_status"]:
        if (
            ...
            raise FormalPlanningProcessError("Planning mount receipt item is invalid")
    ...
    if not required_mounts <= mount_destinations:
        raise FormalPlanningProcessError("Planning required mount receipt is incomplete")
    ...
    if not {"/policy", str(SANDBOX_ASSET_ROOT), "/runtime"} <= readonly_destinations:
        raise FormalPlanningProcessError("Planning policy mounts are not read-only")
    if type(value["open_fds"]) is not list:
        raise FormalPlanningProcessError("Planning descriptor receipt is invalid")
    ...
    if len({item["fd"] for item in all_fds}) != len(all_fds):
        raise FormalPlanningProcessError("Planning descriptor numbers are repeated")
    ...
    if [item.get("kind") for item in core_fds] != [
        ...
        raise FormalPlanningProcessError("Planning descriptor kinds are invalid")
    if (
        ...
        raise FormalPlanningProcessError("Planning descriptor numbers are invalid")
    if (value["model_sha256"] is None) != (value["model_file_count"] == 0):
        raise FormalPlanningProcessError("Planning model commitment/count disagree")
    if value["model_load_count"] != (1 if value["model_sha256"] else 0):
        raise FormalPlanningProcessError("Planning model load receipt disagrees")
    modules = value["loaded_policy_modules"]
    if type(modules) is not list or modules != sorted(set(modules)):
        raise FormalPlanningProcessError("Planning loaded-module receipt is invalid")
    ...
    if (
        ...
        raise FormalPlanningProcessError("Planning system runtime receipt is invalid")
    if any(not item["destination"].startswith("/usr/lib/wsl/")
           for item in system_files[len(SYSTEM_RUNTIME_FILES):]):
        raise FormalPlanningProcessError("GPU driver receipt has an invalid path")
    ...
    if (
        ...
        raise FormalPlanningProcessError("Planning fixed identity receipt is invalid")
    return value
...
def _synthetic_runtime_receipt() -> dict[str, Any]:
    ...
    return {
        ...
        "model_load_count": 0,
        "policy_runner": "production_planning_policy_episode",
        "mount_status": [
...
def validate_process_boundary_attestation(value: object) -> dict[str, Any]:
    ...
    if type(value) is not dict or set(value) != expected_keys:
        raise FormalPlanningProcessError(
            "Planning process-boundary attestation has invalid fields"
        )
    ...
    exact_values: dict[str, object] = {
        ...
        "policy_runtime_module": "anchor.integration.planning_policy_runtime",
        "policy_runner": "production_planning_policy_episode",
    ...
    for name, expected in exact_values.items():
        if value.get(name) != expected or type(value.get(name)) is not type(expected):
            raise FormalPlanningProcessError(
                f"Planning process-boundary attestation {name} is invalid"
            )
    ...
    if (
        ...
        raise FormalPlanningProcessError("Planning sandbox launcher attestation is invalid")
    ...
    if (
        ...
        raise FormalPlanningProcessError("Planning policy projection attestation is invalid")
    if type(value["mount_commitments"]) is not dict:
        raise FormalPlanningProcessError("Planning mount commitments are invalid")
    runtime = _validate_runtime_receipt(value["runtime_receipt"])
    if runtime["policy_runner"] != "production_planning_policy_episode":
        raise FormalPlanningProcessError(
    ...
    if accelerator:
        ...
        if mounts.get("accelerator_runtime") != {
            ...
            raise FormalPlanningProcessError("Planning GPU mount commitment is invalid")
        if runtime["environment_values"] != environment:
            raise FormalPlanningProcessError("Planning GPU environment differs from runtime")
    if set(mounts) != expected_mount_fields:
        raise FormalPlanningProcessError("Planning mount commitment fields are invalid")
    ...
    if (
        ...
        raise FormalPlanningProcessError("Planning source/gallery commitments disagree")
    model_mount = mounts["model_repository"]
    if runtime["model_sha256"] is None:
        if model_mount is not None:
            raise FormalPlanningProcessError("Planning unexpected model mount commitment")
    elif (
        ...
        raise FormalPlanningProcessError("Planning model mount commitment disagrees")
    ...
    if (
        ...
        raise FormalPlanningProcessError("Planning runtime/private mount commitment is invalid")
    ...
    if (
        ...
        raise FormalPlanningProcessError("Planning live-process attestation is invalid")
    episode = value["latest_episode_receipt"]
    if episode is not None:
        _validate_episode_receipt(episode)
        if episode["worker_nonce"] != runtime["worker_nonce"]:
            raise FormalPlanningProcessError("episode receipt belongs to another worker")
...
def _build_process_boundary_attestation(
    ...
    *,
    ...
    if child_environment_fields is not None and sorted(child_environment_fields) != sorted(environment):
        raise FormalPlanningProcessError("child environment field list is not the fixed contract")
...
def _child_loop(
    ...
    *,
    ...
    bundle_build_count += 1
    from anchor.integration.planning_policy_runtime import (
        run_planning_policy_episode,
        run_planning_policy_test_episode,
    )

    policy_runner = (
        run_planning_policy_test_episode
        if test_policy
        else run_planning_policy_episode
    )
    policy_runner_name = (
        "embedded_test_fixture"
        if test_policy
        else "production_planning_policy_episode"
    )
        ...
        while True:
            ...
            if message.get("type") == "shutdown":
                if message != {
                    ...
                    raise FormalPlanningProcessError("invalid or replayed shutdown message")
                channel.send_json(
                    {
                        "schema": PROCESS_MESSAGE_SCHEMA,
                        "type": "closed",
            ...
            if not _hash_valid(episode_nonce) or episode_nonce in used_nonces:
                raise FormalPlanningProcessError("episode nonce is invalid or replayed")
            transcript = _Transcript(
                worker_nonce=worker_nonce,
            ...
            if (
                ...
                raise FormalPlanningProcessError("invalid start_episode message")
...
def _verify_live_child(
    ...
    if process.poll() is not None:
        raise FormalPlanningProcessError("sandbox child exited before live verification")
...
class FormalPlanningPolicyProcess:
    ...
    def __init__(
        ...
        *,
        ...
        if not executable.is_file():
            raise ValueError("formal Planning Python executable must be a file")
        ...
        try:
            ...
            if (
                ...
                raise FormalPlanningProcessError(
                    "formal Planning child startup failed: "
            ...
            if type(ready) is not dict or set(ready) != {
                ...
                raise FormalPlanningProcessError("formal Planning child returned invalid ready")
            ...
            if runtime_receipt["source_sha256"] != self._projection.source_sha256:
                raise FormalPlanningProcessError("child policy projection hash differs")
            if runtime_receipt["gallery_sha256"] != self._projection.gallery_sha256:
                raise FormalPlanningProcessError("child gallery projection hash differs")
            if runtime_receipt["model_sha256"] != self._projection.model_sha256:
                raise FormalPlanningProcessError("child model repository hash differs")
            ...
            for item in runtime_receipt["system_runtime_files"][len(SYSTEM_RUNTIME_FILES):]:
                if item["sha256"] != sha256_file(item["destination"]):
                    raise FormalPlanningProcessError("GPU driver hash differs from host runtime")
            expected_runner = (
                "embedded_test_fixture"
                if test_policy
                else "production_planning_policy_episode"
            ...
            parent_socket.close()
            self._abort()
            if isinstance(exc, FormalPlanningProcessError):
                raise
    ...
    def run_episode(
        ...
        *,
        ...
        if (
            ...
            raise FormalPlanningProcessError("formal Planning child is unavailable")
        ...
        while True:
            try:
                ...
                if isinstance(exc, FormalPlanningProcessError):
                    raise
                raise FormalPlanningProcessError(
                    f"formal Planning child disconnected: {exc}"
                ) from exc
            message_type = core.get("type")
            if message_type == "action":
                if set(core) != {"type", "action"}:
                    self._abort()
                    raise FormalPlanningProcessError("child action has invalid fields")
                if actions >= step_budget:
                    self._abort()
                    raise FormalPlanningProcessError("child exceeded its fixed action horizon")
                try:
                    action = deserialize_policy_action(core["action"])
                except PolicyIPCValidationError as exc:
                    self._abort()
                    raise FormalPlanningProcessError(f"child action is invalid: {exc}") from exc
            ...
            if message_type == "error":
                if set(core) != {"type", "exception_type", "detail"}:
                    self._abort()
                    raise FormalPlanningProcessError("child error message is malformed")
                self._abort()
                raise FormalPlanningProcessError(
            ...
            try:
                ...
                if type(child_receipt) is not dict:
                    raise FormalPlanningProcessError("child episode receipt is invalid")
                complete_receipt = {
                    **child_receipt,
    ...
    def boundary_attestation(self) -> dict[str, Any]:
        ...
        if self._runtime_receipt["policy_runner"] != (
            "production_planning_policy_episode"
        ):
            raise FormalPlanningProcessError(
    ...
    def close(self) -> None:
        ...
        try:
            ...
            if reply != {
                ...
                raise FormalPlanningProcessError("child returned invalid close receipt")
    ...
    def __enter__(self) -> "FormalPlanningPolicyProcess":
        return self
