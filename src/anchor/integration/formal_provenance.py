    ...
    *,
    ...
    return {
        ...
        "runtime_binding": {
            ...
            "planning_process_plan_binding": dependencies["planning_process"],
        },
...
def validate_shard_provenance(payload: Mapping[str, Any]) -> None:
    ...
    planning_binding = runtime_binding.get("planning_process_plan_binding")
    if (
        not isinstance(planning_binding, Mapping)
        or planning_binding.get("schema")
        != "libero-formal-planning-plan-binding.v1"
    ):
        raise FormalProvenanceError("provenance Planning process binding is invalid")
