...
sys.argv[0] = "anchor.integration"
runpy.run_module("anchor.integration", run_name="__main__")
"""
...
def planning_process_dependency_provenance(
    *,
    asset_root: str | Path,
    perception_model: str | Path,
    python_executable: str | Path,
) -> dict[str, Any]:
    """Derive every immutable Planning sandbox commitment from plan inputs."""
    ...
    with tempfile.TemporaryDirectory(prefix="libero-planning-plan-") as temporary:
        ...
        for relative in POLICY_SOURCE_FILES:
            ...
            if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
                raise CampaignPlanError(
                    f"Planning projected source is not an exact regular file: {source}"
                )
        ...
        for source in gallery_sources:
            ...
            if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
                raise CampaignPlanError(
                    f"Planning gallery source is not an exact regular file: {source}"
                )
    ...
    identity_payloads = {
        "/etc/passwd": b"nobody:x:65534:65534:Planning policy:/tmp:/usr/sbin/nologin\n",
    ...
    return {
        "schema": "libero-formal-planning-plan-binding.v1",
        **bound,
        "sha256": structured_sha256(
            "libero-formal-planning-plan-binding.v1", bound
        ),
    }
...
def collect_dependency_provenance(
    *,
    ...
    return {
        ...
        "planning_process": planning_process_dependency_provenance(
...
def validate_campaign_plan(payload: Mapping[str, Any]) -> None:
    ...
    planning_process = dependencies.get("planning_process")
    if (
        not isinstance(planning_process, Mapping)
        or planning_process.get("schema")
        != "libero-formal-planning-plan-binding.v1"
    ):
        raise CampaignPlanError("plan Planning process binding is invalid")
    planning_bound = {
        key: planning_process.get(key)
        for key in planning_process
        if key not in {"schema", "sha256"}
    }
    if planning_process.get("sha256") != structured_sha256(
        "libero-formal-planning-plan-binding.v1", planning_bound
    ):
        raise CampaignPlanError("plan Planning process binding hash is invalid")
    for field in (
        "source_sha256",
        ...
        value = planning_process.get(field)
        if not isinstance(value, str) or _SHA256.fullmatch(value) is None:
            raise CampaignPlanError(f"plan Planning {field} is invalid")
    for field in ("source_file_count", "gallery_file_count", "model_file_count"):
        value = planning_process.get(field)
        if not isinstance(value, int) or isinstance(value, bool) or value < 1:
            raise CampaignPlanError(f"plan Planning {field} is invalid")
    if (
        planning_process.get("python_sha256") != executable_digest
        or planning_process.get("bwrap_path") != "/usr/bin/bwrap"
        or planning_process.get("policy_runner")
        != "production_planning_policy_episode"
        or planning_process.get("bundle_build_count") != 1
        or planning_process.get("model_load_count") != 1
        or not isinstance(planning_process.get("system_runtime_files"), list)
        or not isinstance(planning_process.get("fixed_identity_files"), list)
        or not isinstance(planning_process.get("environment_values"), Mapping)
    ):
        raise CampaignPlanError("plan Planning process fixed values are invalid")
    ...
    if (
        ...
        raise CampaignPlanError(
            ...
            "sequential ANCHOR then Planning execution"
        )
