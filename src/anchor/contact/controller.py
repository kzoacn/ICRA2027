"""Closed-loop ANCHOR-style controller for LIBERO-Goal contact skills."""
...
class GoalControllerConfig:
    ...
    # task-independent load proof used by Planning and does not infer contact
    ...
    # ANCHOR's hinge is reconstructed from a closed-handle point plus a
...
class GoalContactPolicy:
    ...
    def act(self, observation: RobotObservation) -> PolicyDecision:
        ...
        if step.kind is GoalSkillKind.ANCHOR_PLACE_IN:
            self._status = GoalExecutorStatus.HANDOFF
            self._phase = "anchor_handoff"
            self._message = f"handoff to ANCHOR: put {step.subject} in {step.target}"
            return self._decision(OSCAction.hold(-1.0), self._message, stop=True)
    ...
    def _act_microwave(
        ...
        if (step.kind is GoalSkillKind.CLOSE_MICROWAVE
            ...
            from ..manipulation.microwave_close import MicrowaveFaceCloser
        ...
        if self._phase == "detect":
            ...
            if step.kind is GoalSkillKind.OPEN_MICROWAVE:
                ...
                if rotary is None:
                    ...
                    # but the ANCHOR RGB-D provider supplies full OBB-derived
    ...
    def _microwave_reacquire_open_edge(
        ...
        The production ANCHOR wrapper exposes the original RGB-D appliance
        ...
        if callable(append_diagnostic):
            append_diagnostic(
                {
                    "kind": "anchor_microwave_local_reacquisition",
    ...
    def _microwave_rotary_geometry(
        ...
        *,
        ...
        ANCHOR wraps the reusable microwave feature detector and freezes its
        ...
        for provider in providers:
            ...
            # door.  For the standard ANCHOR wrapper, mirror the provider's
            ...
            if callable(append_diagnostic):
                append_diagnostic(
                    {
                        "kind": "anchor_microwave_articulation_geometry",
    ...
    def _complete(self, message: str) -> PolicyDecision:
        ...
        if self._step_index < len(self._plan.steps):
            ...
            if next_step.kind is GoalSkillKind.ANCHOR_PLACE_IN:
                self._status = GoalExecutorStatus.HANDOFF
                self._phase = "anchor_handoff"
                self._message = (
                    f"{message}; handoff to ANCHOR: put {next_step.subject} in {next_step.target}"
                )
                return self._decision(OSCAction.hold(-1.0), self._message, stop=True)
            self._set_phase("detect")
