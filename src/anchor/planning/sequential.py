"""Instruction-plan execution for Planning.
...
types fail closed.  Pick/place goals retain Planning's constraint graph and
...
class MPCContactGoalExecutor:
    """Execute sensor-derived fixture goals with Planning continuous MPC."""
    ...
    def __init__(
        ...
        config: PlanningContactConfig | None = None,
    ) -> None:
        self.observer = observer
        self.robot = robot
        self.mpc = mpc
        self.provider = provider
        self.config = config or PlanningContactConfig()
    ...
    def set_final_goal_context(self, is_final_goal: bool) -> None:
        ...
        This is supplied by :class:`PlanningSequentialCoordinator`; it is not
    ...
    def execute(self, goal: AtomicGoal) -> ContactGoalResult:
        ...
        try:
            ...
            if callable(set_candidate):
                set_candidate(f"planning-contact-{goal.kind.value}")
...
class PlanningSequentialCoordinator:
    ...
    def __init__(
        ...
        *,
        planner: PlanningTaskPlanner | None = None,
        ...
        self.planner = planner or PlanningTaskPlanner()
    ...
    def run(self, instruction: str) -> PlanningSequenceResult:
        ...
        for index, goal in enumerate(plan.goals):
            ...
            if goal.kind in AtomicGoalGraphCompiler.MOTION_KINDS:
                ...
                try:
                    result: PlanningResult = self.motion_controller.run(instruction)
                ...
                record = GoalExecutionRecord(
                    ...
                    goal,
                    False,
                    f"no Planning executor for {goal.kind.value!r}",
                    (),
                )
            if (
                record.success
        ...
        return PlanningSequenceResult(True, instruction, plan, tuple(records))
