"""Executable phased Planning controller with MPC replanning and recovery."""
...
class TerminalExecutionError(RuntimeError):
    """Stop a Planning run immediately while preserving its attempt journal."""
...
class PlanningControllerConfig:
    ...
    # fixture's partial RGB-D OBB remains unsafe to bind.  Planning may move an
...
class PlanningController:
    ...
    def __init__(
        ...
        config: PlanningControllerConfig | None = None,
    ) -> None:
        ...
        self.config = config or PlanningControllerConfig()
    ...
    def run(self, task_text: str) -> PlanningResult:
                    ...
                    for phase_attempt in range(1, self.config.max_phase_attempts + 1):
                        try:
                            ...
                            if phase == Phase.GRASP:
                                ...
                                # Planning's successful LIFT is the first
        ...
        self._reject_pending_grasp(last_failure or "planning run failed")
        return PlanningResult(
            False,
            task_text,
            tuple(attempts),
    ...
    def _matching_selector_view_hint(
        ...
        if scene.capture_id:
            ...
            # fallback only; formal Planning requires a populated capture_id.
            try:
