...
class ControllerConfig:
    ...
    # Use the Planning-matched cavity dynamics only while the current attempt
...
class AnchorController:
    ...
    def clear_episode_state(self) -> None:
        ...
        activation.  The ANCHOR dispatcher uses it to clear an inactive
    ...
    def activate_after_episode_clear(self, task: str | TaskSpec) -> TaskSpec:
        ...
        if (
            ...
            raise RuntimeError(
                "ANCHOR controller activation requires freshly cleared state"
            )
        spec = (
        ...
        if self._phase == "cavity_preshape":
            ...
            if self._cavity_rim_preshape_ready(
                ...
                if self._cavity_level_pose is not None:
                    ...
                    # waypoint.  Mirror Planning's proven mechanics: command
    ...
    def _selector_cavity_reference(self, step: SkillStep) -> object | None:
        ...
        compatible with the small ANCHOR perception protocol.  The compiled
    ...
    def _sensor_geometry_trace(value: object | None) -> dict[str, object] | None:
        ...
        if observed is not None and observed is not value:
            observed_trace = AnchorController._sensor_geometry_trace(observed)
    ...
    def _rim_pinch_target(
        ...
        In ANCHOR's raw robosuite EEF-body orientation, Panda's two pads lie
